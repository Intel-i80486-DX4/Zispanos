...
@Info(name = "HotBarRefill", category = Category.MISC, description = "Refills your Hotbar")
public class AutoRefill extends Module
{
    ...
    private static Map<Integer, ItemStack> getInventorySlots(int current, final int last) {
        ...
        while (current <= last) {
            fullInventorySlots.put(current, (ItemStack)AutoRefill.mc.player.inventoryContainer.getInventory().get(current));
            ++current;
        }
    ...
    public void onUpdate() {
        if (AutoRefill.mc.player == null) {
            return;
        }
        if (AutoRefill.mc.currentScreen instanceof GuiContainer) {
            return;
        }
        ...
        AutoRefill.mc.playerController.windowClick(0, inventorySlot, 0, ClickType.PICKUP, (EntityPlayer)AutoRefill.mc.player);
        AutoRefill.mc.playerController.windowClick(0, hotbarSlot, 0, ClickType.PICKUP, (EntityPlayer)AutoRefill.mc.player);
        AutoRefill.mc.playerController.windowClick(0, inventorySlot, 0, ClickType.PICKUP, (EntityPlayer)AutoRefill.mc.player);
    }
    ...
    private Pair<Integer, Integer> findReplenishableHotbarSlot() {
        ...
        for (final Map.Entry<Integer, ItemStack> hotbarSlot : getHotbar().entrySet()) {
            ...
            if (!stack.isEmpty) {
                ...
                returnPair = (Pair<Integer, Integer>)new Pair((Object)inventorySlot, (Object)hotbarSlot.getKey());
            }
        }
        return returnPair;
    }
    ...
    private int findCompatibleInventorySlot(final ItemStack hotbarStack) {
        ...
        for (final Map.Entry<Integer, ItemStack> entry : getInventory().entrySet()) {
            ...
            if (!inventoryStack.isEmpty) {
                ...
                final int currentStackSize = ((ItemStack)AutoRefill.mc.player.inventoryContainer.getInventory().get((int)entry.getKey())).stackSize;
