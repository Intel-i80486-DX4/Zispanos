...
@Info(name = "UltraCrystalSoul", category = Category.CRYSTALS)
public class UltraCrystal extends Module
{
    ...
    public UltraCrystal() {
        ...
        this.tempo_de_colocar = this.register(Settings.d("Place Delay", 1.0));
        this.tempo_de_ataque = this.register(Settings.d("Hit Delay", 1.0));
        ...
        this.packetListener = (Listener<PacketEvent.Send>)new Listener(event -> {
            ...
            if (packet instanceof CPacketPlayer && UltraCrystal.is_spoofing_angles) {
                ((CPacketPlayer)packet).yaw = (float)UltraCrystal.player_yaw;
                ((CPacketPlayer)packet).pitch = (float)UltraCrystal.player_pitch;
            }
        }, new Predicate[0]);
    }
    ...
    public void onEnable() {
        if (this.prefixo_chat.getValue()) {
            Command.sendChatMessage("UltraCrystalSoul <- " + ChatFormatting.GREEN + "Enabled!");
        }
    }
    ...
    public void onDisable() {
        ...
        reset_rotation();
        if (this.prefixo_chat.getValue()) {
            Command.sendChatMessage("UltraCrystalSoul -> " + ChatFormatting.RED + "Disabled!");
        }
    }
    
    @Override
    public void onUpdate() {
        final EntityEnderCrystal crystal = (EntityEnderCrystal)UltraCrystal.mc.world.loadedEntityList.stream().filter(entity -> entity instanceof EntityEnderCrystal).map(entity -> entity).min(Comparator.comparing(d -> UltraCrystal.mc.player.getDistance(d))).orElse(null);
        if (this.explodir.getValue() && crystal != null && UltraCrystal.mc.player.getDistance((Entity)crystal) <= this.alcance.getValue()) {
            if (System.nanoTime() / 1000000L - this.system_time >= this.tempo_de_ataque.getValue()) {
                if (this.ant_fraqueza.getValue() && UltraCrystal.mc.player.isPotionActive(MobEffects.WEAKNESS)) {
                    if (!this.attack) {
                ...
                this.lookAtPacket(crystal.posX, crystal.posY, crystal.posZ, (EntityPlayer)UltraCrystal.mc.player);
                UltraCrystal.mc.playerController.attackEntity((EntityPlayer)UltraCrystal.mc.player, (Entity)crystal);
                UltraCrystal.mc.player.swingArm(EnumHand.MAIN_HAND);
        ...
        int crystal_slot = (UltraCrystal.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL) ? UltraCrystal.mc.player.inventory.currentItem : -1;
        if (crystal_slot == -1) {
            for (int l = 0; l < 9; ++l) {
                if (UltraCrystal.mc.player.inventory.getStackInSlot(l).getItem() == Items.END_CRYSTAL) {
                    crystal_slot = l;
                    break;
                }
            }
        ...
        if (UltraCrystal.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL) {
            off_hand = true;
        }
        ...
        entities.addAll((Collection<? extends Entity>)UltraCrystal.mc.world.playerEntities.stream().filter(entityPlayer -> !Friends.isFriend(entityPlayer.getName())).collect(Collectors.toList()));
        BlockPos q = null;
        double damage = 0.5;
        for (final Entity entity_two : entities) {
            if (entity_two != UltraCrystal.mc.player) {
                ...
                for (final BlockPos blockPos : blocks) {
                    ...
                    final double self = calculate_damage(blockPos.x + 0.5, blockPos.y + 1, blockPos.z + 0.5, (Entity)UltraCrystal.mc.player);
                    if (self > d2 && d2 >= ((EntityLivingBase)entity_two).getHealth()) {
                        continue;
                    }
                    if (self - 0.5 > UltraCrystal.mc.player.getHealth()) {
                        continue;
                    }
        ...
        if (this.colocar.getValue()) {
            if (!off_hand && UltraCrystal.mc.player.inventory.currentItem != crystal_slot) {
                if (this.auto_switch.getValue()) {
                    UltraCrystal.mc.player.inventory.currentItem = crystal_slot;
                    reset_rotation();
                    this.switch_cooldown = true;
                }
                return;
            }
            this.lookAtPacket(q.x + 0.5, q.y - 0.5, q.z + 0.5, (EntityPlayer)UltraCrystal.mc.player);
            EnumFacing f;
            if (this.raytrace.getValue()) {
                final RayTraceResult result = UltraCrystal.mc.world.rayTraceBlocks(new Vec3d(UltraCrystal.mc.player.posX, UltraCrystal.mc.player.posY + UltraCrystal.mc.player.getEyeHeight(), UltraCrystal.mc.player.posZ), new Vec3d(q.x + 0.5, q.y - 0.5, q.z + 0.5));
            ...
            if (System.nanoTime() / 1000000L - this.system_time >= this.tempo_de_colocar.getValue()) {
                UltraCrystal.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(q, f, off_hand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                ++this.places;
                this.system_time = System.nanoTime() / 1000000L;
            }
        }
        if (UltraCrystal.is_spoofing_angles) {
            if (UltraCrystal.toggle_pitch) {
                final EntityPlayerSP player;
                final EntityPlayerSP player = player = UltraCrystal.mc.player;
                player.rotationPitch += 4.0E-4f;
                UltraCrystal.toggle_pitch = false;
            }
            else {
                final EntityPlayerSP player2;
                final EntityPlayerSP player = player2 = UltraCrystal.mc.player;
                player2.rotationPitch -= 4.0E-4f;
                UltraCrystal.toggle_pitch = true;
            }
        }
    }
    ...
    private boolean can_place_crystal(final BlockPos blockPos) {
        ...
        return (UltraCrystal.mc.world.getBlockState(blockPos).getBlock() == Blocks.BEDROCK || UltraCrystal.mc.world.getBlockState(blockPos).getBlock() == Blocks.OBSIDIAN) && UltraCrystal.mc.world.getBlockState(boost).getBlock() == Blocks.AIR && UltraCrystal.mc.world.getBlockState(boost2).getBlock() == Blocks.AIR && UltraCrystal.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost)).isEmpty() && UltraCrystal.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost2)).isEmpty();
    }
    
    public static BlockPos get_player_pos() {
        return new BlockPos(Math.floor(UltraCrystal.mc.player.posX), Math.floor(UltraCrystal.mc.player.posY), Math.floor(UltraCrystal.mc.player.posZ));
    }
    ...
    public static float calculate_damage(final double posX, final double posY, final double posZ, final Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            finald = get_blast_reduction((EntityLivingBase)entity, get_damage_multiplied(damage), new Explosion((World)UltraCrystal.mc.world, (Entity)null, posX, posY, posZ, 6.0f, false, true));
        }
        return (float)finald;
    }
    ...
    private static void reset_rotation() {
        if (UltraCrystal.is_spoofing_angles) {
            UltraCrystal.player_yaw = UltraCrystal.mc.player.rotationYaw;
            UltraCrystal.player_pitch = UltraCrystal.mc.player.rotationPitch;
            UltraCrystal.is_spoofing_angles = false;
        }
    }
    
    private static void set_yaw_pitch(final float player_yaw_, final float player_pitch_) {
        UltraCrystal.player_yaw = player_yaw_;
        UltraCrystal.player_pitch = player_pitch_;
        UltraCrystal.is_spoofing_angles = true;
    }
