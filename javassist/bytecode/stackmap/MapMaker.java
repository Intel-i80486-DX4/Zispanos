    ...
    private void traceException(final byte[] code, BasicBlock.Catch handler) throws BadBytecode {
        while (handler != null) {
            ...
            if (tb.alreadySet()) {
                ...
                tb.stackTypes[0] = this.merge((TypeData)this.toExceptionType(handler.typeIndex), tb.stackTypes[0]);
            }
            else {
        ...
        for (final TypedBlock block : blocks) {
            if (block.alreadySet()) {
                for (int n = block.localsTypes.length, j = 0; j < n; ++j) {
                    index = block.localsTypes[j].dfs((List)preOrder, index, this.classPool);
                }
                for (int n = block.stackTop, j = 0; j < n; ++j) {
                    index = block.stackTypes[j].dfs((List)preOrder, index, this.classPool);
                }
            }
        }
    }
