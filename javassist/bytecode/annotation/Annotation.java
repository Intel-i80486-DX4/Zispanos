...
public class Annotation
{
    ...
    public static MemberValue createMemberValue(final ConstPool cp, final CtClass type) throws NotFoundException {
        if (type == CtClass.booleanType) {
            return (MemberValue)new BooleanMemberValue(cp);
        }
        if (type == CtClass.byteType) {
            return (MemberValue)new ByteMemberValue(cp);
        }
        if (type == CtClass.charType) {
            return (MemberValue)new CharMemberValue(cp);
        }
        if (type == CtClass.shortType) {
            return new ShortMemberValue(cp);
        }
        if (type == CtClass.intType) {
            return (MemberValue)new IntegerMemberValue(cp);
        }
        if (type == CtClass.longType) {
            return (MemberValue)new LongMemberValue(cp);
        }
        if (type == CtClass.floatType) {
            return (MemberValue)new FloatMemberValue(cp);
        }
        if (type == CtClass.doubleType) {
            return (MemberValue)new DoubleMemberValue(cp);
        }
        if (type.getName().equals("java.lang.Class")) {
            return (MemberValue)new ClassMemberValue(cp);
        }
        if (type.getName().equals("java.lang.String")) {
            return (MemberValue)new StringMemberValue(cp);
        }
        if (type.isArray()) {
            final CtClass arrayType = type.getComponentType();
            final MemberValue member = createMemberValue(cp, arrayType);
            return (MemberValue)new ArrayMemberValue(member, cp);
        }
        if (type.isInterface()) {
            final Annotation info = new Annotation(cp, type);
            return (MemberValue)new AnnotationMemberValue(info, cp);
        }
        final EnumMemberValue emv = new EnumMemberValue(cp);
        emv.setType(type.getName());
        return (MemberValue)emv;
    }
    ...
    public Object toAnnotationType(final ClassLoader cl, final ClassPool cp) throws ClassNotFoundException, NoSuchClassError {
        final Class<?> clazz = (Class<?>)MemberValue.loadClass(cl, this.getTypeName());
        try {
            return AnnotationImpl.make(cl, (Class)clazz, cp, this);
        }
