...
public final class ClassFile
{
    ...
    public ClassFile(final boolean isInterface, final String classname, final String superclass) {
        ...
        (this.attributes = new ArrayList<AttributeInfo>()).add((AttributeInfo)new SourceFileAttribute(this.constPool, getSourcefileName(this.thisclassname)));
    }
    ...
    public void compact() {
        ...
        this.attributes = (List<AttributeInfo>)AttributeInfo.copyAll((List)this.attributes, cp);
        this.constPool = cp;
    }
    ...
    public final void renameClass(String oldname, String newname) {
        ...
        for (final FieldInfo finfo : this.fields) {
            ...
            AttributeInfo.renameClass((List)finfo.getAttributes(), oldname, newname);
        }
    }
    ...
    public final void renameClass(final Map<String, String> classnames) {
        ...
        this.constPool.renameClass((Map)classnames);
        AttributeInfo.renameClass((List)this.attributes, (Map)classnames);
        for (final MethodInfo minfo : this.methods) {
            final String desc = minfo.getDescriptor();
            minfo.setDescriptor(Descriptor.rename(desc, (Map)classnames));
            AttributeInfo.renameClass(minfo.getAttributes(), (Map)classnames);
        }
        for (final FieldInfo finfo : this.fields) {
            final String desc = finfo.getDescriptor();
            finfo.setDescriptor(Descriptor.rename(desc, (Map)classnames));
            AttributeInfo.renameClass((List)finfo.getAttributes(), (Map)classnames);
        }
    }
    
    public final void getRefClasses(final Map<String, String> classnames) {
        this.constPool.renameClass((Map)classnames);
        AttributeInfo.getRefClasses((List)this.attributes, (Map)classnames);
        for (final MethodInfo minfo : this.methods) {
            final String desc = minfo.getDescriptor();
            Descriptor.rename(desc, (Map)classnames);
            AttributeInfo.getRefClasses(minfo.getAttributes(), (Map)classnames);
        }
        for (final FieldInfo finfo : this.fields) {
            final String desc = finfo.getDescriptor();
            Descriptor.rename(desc, (Map)classnames);
            AttributeInfo.getRefClasses((List)finfo.getAttributes(), (Map)classnames);
        }
    }
    ...
    public AttributeInfo removeAttribute(final String name) {
        return AttributeInfo.remove((List)this.attributes, name);
    }
    
    public void addAttribute(final AttributeInfo info) {
        AttributeInfo.remove((List)this.attributes, info.getName());
    ...
    public void write(final DataOutputStream out) throws IOException {
        ...
        AttributeInfo.writeAll((List)this.attributes, out);
    }
