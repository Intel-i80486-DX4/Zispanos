    ...
    static AttributeInfo read(final ConstPool cp, final DataInputStream in) throws IOException {
        ...
        if (first < 'E') {
            ...
            if (nameStr.equals("BootstrapMethods")) {
                return (AttributeInfo)new BootstrapMethodsAttribute(cp, name, in);
            }
            if (nameStr.equals("Code")) {
                return (AttributeInfo)new CodeAttribute(cp, name, in);
            }
        ...
        if (first < 'M') {
            ...
            if (nameStr.equals("LineNumberTable")) {
                return (AttributeInfo)new LineNumberAttribute(cp, name, in);
            }
            if (nameStr.equals("LocalVariableTable")) {
                return (AttributeInfo)new LocalVariableAttribute(cp, name, in);
            }
            if (nameStr.equals("LocalVariableTypeTable")) {
                return (AttributeInfo)new LocalVariableTypeAttribute(cp, name, in);
            }
        }
        if (first < 'S') {
            ...
            if (nameStr.equals("RuntimeVisibleTypeAnnotations") || nameStr.equals("RuntimeInvisibleTypeAnnotations")) {
                return (AttributeInfo)new TypeAnnotationsAttribute(cp, name, in);
            }
        }
        if (first >= 'S') {
