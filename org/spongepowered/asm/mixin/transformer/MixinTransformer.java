...
public class MixinTransformer extends TreeTransformer
{
    ...
    private void printProfilerSummary() {
        ...
        printer.kv("Total mixin time", format, new Object[] { totalMixinTime, threedp.format(totalMixinTime * 0.001) }).add();
        printer.kv("Preparing mixins", format, new Object[] { prepareTime, threedp.format(prepareTime * 0.001) });
        printer.kv("Reading input", format, new Object[] { readTime, threedp.format(readTime * 0.001) });
        printer.kv("Applying mixins", format, new Object[] { applyTime, threedp.format(applyTime * 0.001) });
        printer.kv("Writing output", format, new Object[] { writeTime, threedp.format(writeTime * 0.001) }).add();
        printer.kv("of which", (Object)"");
        printer.kv("Time spent loading from disk", format, new Object[] { loadTime, threedp.format(loadTime * 0.001) });
        printer.kv("Time spent transforming classes", format, new Object[] { transformTime, threedp.format(transformTime * 0.001) }).add();
        if (worstTransformer != null) {
            ...
            printer.kv("Time spent", "%s seconds", new Object[] { worstTransformer.getTotalSeconds() });
            printer.kv("called", "%d times", new Object[] { worstTransformer.getTotalCount() }).add();
        }
        printer.kv("   Time allocation:     Processing mixins", "%9d ms %10s%% of total)", new Object[] { actualTime, onedp.format(timeSliceMixin) });
        printer.kv("Loading classes", "%9d ms %10s%% of total)", new Object[] { loadTime, onedp.format(timeSliceLoad) });
        printer.kv("Running transformers", "%9d ms %10s%% of total)", new Object[] { transformTime, onedp.format(timeSliceTransform) });
        if (exportTime > 0L) {
            printer.kv("Exporting classes (debug)", "%9d ms %10s%% of total)", new Object[] { exportTime, onedp.format(timeSliceExport) });
        }
        printer.add();
        try {
            ...
            for (final Map.Entry<String, Long> entry : times.entrySet()) {
                ...
                if (mixinTime > 0L) {
                    printer.add("%-" + longest + "s %8s ms %8s ms in mixin)", new Object[] { name, entry.getValue() + mixinTime, "(" + mixinTime });
                }
                else {
    ...
    public synchronized byte[] transformClassBytes(final String name, final String transformedName, byte[] basicClass) {
        ...
        if (basicClass == null) {
            for (final IClassGenerator generator : this.extensions.getGenerators()) {
                final Profiler.Section genTimer = this.profiler.begin(new String[] { "generator", generator.getClass().getSimpleName().toLowerCase() });
    ...
    private void handleMixinError(final String context, final InvalidMixinException ex, final MixinEnvironment environment, final ErrorPhase errorPhase) throws Error {
        ...
        if (environment.getOption(MixinEnvironment.Option.DEBUG_VERBOSE)) {
            new PrettyPrinter().add("Invalid Mixin").centre().hr('-').kvWidth(10).kv("Action", (Object)errorPhase.name()).kv("Mixin", (Object)mixin.getClassName()).kv("Config", (Object)config.getName()).kv("Phase", (Object)phase).hr('-').add("    %s", new Object[] { ex.getClass().getName() }).hr('-').addWrapped("    %s", new Object[] { ex.getMessage() }).hr('-').add((Throwable)ex, 8).trace(action.logLevel);
        }
