...
public class CallbackInjector extends Injector
{
    ...
    private void inject(final Callback callback) {
        ...
        if (!callback.checkDescriptor(this.methodNode.desc)) {
            ...
            if (callback.canCaptureLocals) {
                ...
                if (surrogateHandler != null && Annotations.getVisible(surrogateHandler, (Class)Surrogate.class) != null) {
                ...
                if (surrogateHandler2 == null || Annotations.getVisible(surrogateHandler2, (Class)Surrogate.class) == null) {
    ...
    private void printLocals(final Callback callback) {
        ...
        printer.kv("Target Method", (Object)methodSig);
        printer.kv("Target Max LOCALS", (Object)callback.target.getMaxLocals());
        printer.kv("Initial Frame Size", (Object)callback.frameSize);
        ...
        if (callback.locals.length > callback.frameSize) {
            ...
            for (int l = 0; l < callback.locals.length; ++l) {
                ...
                if (callback.locals[l] != null) {
                    printer.add("%s [%3d]  %20s  %-50s %s", new Object[] { marker, l, SignaturePrinter.getTypeName(callback.localTypes[l], false), meltSnowman(l, callback.locals[l].name), (l >= callback.frameSize) ? "<capture>" : "" });
                }
                else {
                    final boolean isTop = l > 0 && callback.localTypes[l - 1] != null && callback.localTypes[l - 1].getSize() > 1;
                    printer.add("%s [%3d]  %20s", new Object[] { marker, l, isTop ? "<top>" : "-" });
                }
            }
            printer.hr();
        }
    ...
    private void invokeCallback(final Callback callback, final MethodNode callbackMethod) {
        ...
        if (callback.captureArgs()) {
            Bytecode.loadArgs(callback.target.arguments, (InsnList)callback, (int)(this.isStatic ? 0 : 1), -1);
        }
        this.loadOrCreateCallbackInfo(callback);
        if (callback.canCaptureLocals) {
            Locals.loadLocals(callback.localTypes, (InsnList)callback, callback.frameSize, callback.extraArgs);
        }
        this.invokeHandler((InsnList)callback, callbackMethod);
    }
        ...
        boolean checkDescriptor(final String desc) {
            ...
            for (int arg = 0; arg < myTypes.length; ++arg) {
                ...
                if (!type.equals(myTypes[arg])) {
                    ...
                    if (Annotations.getInvisibleParameter(this.handler, (Class)Coerce.class, arg) == null) {
                        return false;
                    }
