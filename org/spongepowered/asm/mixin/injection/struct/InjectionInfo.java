...
public abstract class InjectionInfo extends SpecialMethodInfo implements ISliceContext
{
    ...
    protected Set<MemberInfo> parseTargets(final String type) {
        final List<String> methods = (List<String>)Annotations.getValue(this.annotation, "method", false);
        if (methods == null) {
        ...
        for (final String method : methods) {
            try {
                final MemberInfo targetMember = MemberInfo.parseAndValidate(method, (IMixinContext)this.mixin);
    ...
    protected List<AnnotationNode> readInjectionPoints(final String type) {
        final List<AnnotationNode> ats = (List<AnnotationNode>)Annotations.getValue(this.annotation, this.atKey, false);
        if (ats == null) {
    ...
    protected void parseRequirements() {
        ...
        final Integer expect = (Integer)Annotations.getValue(this.annotation, "expect");
        if (expect != null) {
            this.expectedCallbackCount = expect;
        }
        final Integer require = (Integer)Annotations.getValue(this.annotation, "require");
        ...
        final Integer allow = (Integer)Annotations.getValue(this.annotation, "allow");
        if (allow != null) {
            ...
            for (int count = 0, pass = 0; pass < passes && count < 1; ++pass) {
                ...
                for (final MethodNode target : this.classNode.methods) {
                    if (member.matches(target.name, target.desc, ordinal)) {
                        final boolean isMixinMethod = Annotations.getVisible(target, (Class)MixinMerged.class) != null;
                        if (member.matchAll) {
    ...
    private void checkTarget(final MethodNode target) {
        final AnnotationNode merged = Annotations.getVisible(target, (Class)MixinMerged.class);
        if (merged == null) {
            return;
        }
        final String owner = (String)Annotations.getValue(merged, "mixin");
        final int priority = (int)Annotations.getValue(merged, "priority");
        ...
        if (Annotations.getVisible(target, (Class)Final.class) != null) {
    ...
    protected String getDynamicInfo() {
        final AnnotationNode annotation = Annotations.getInvisible(this.method, (Class)Dynamic.class);
        String description = Strings.nullToEmpty((String)Annotations.getValue(annotation));
        final Type upstream = (Type)Annotations.getValue(annotation, "mixin");
        if (upstream != null) {
    ...
    public static InjectionInfo parse(final MixinTargetContext mixin, final MethodNode method) {
        ...
        if (annotation.desc.endsWith(ModifyArgs.class.getSimpleName() + ";")) {
            return (InjectionInfo)new ModifyArgsInjectionInfo(mixin, method, annotation);
        }
        if (annotation.desc.endsWith(Redirect.class.getSimpleName() + ";")) {
            return (InjectionInfo)new RedirectInjectionInfo(mixin, method, annotation);
        }
        if (annotation.desc.endsWith(ModifyVariable.class.getSimpleName() + ";")) {
            return (InjectionInfo)new ModifyVariableInjectionInfo(mixin, method, annotation);
        }
