...
class AnnotatedMixinElementHandlerInjector extends AnnotatedMixinElementHandler
{
    ...
    protected final void remapNewTarget(final String subject, final String reference, final AnnotatedElementInjectionPoint elem) {
        ...
        if (target != null) {
            ...
            final ObfuscationData<String> mappings = (ObfuscationData<String>)new ObfuscationData();
            for (final ObfuscationType type : remapped) {
                final MappingMethod mapping = (MappingMethod)remapped.get(type);
                if (desc == null) {
                    mappings.put(type, (Object)mapping.getOwner());
                }
                else {
                    mappings.put(type, (Object)mapping.getDesc().replace(")V", ")L" + mapping.getOwner() + ";"));
                }
            }
