...
public class Constructor extends SafeConstructor
{
    ...
    public Constructor(final Class<?> theRoot) {
        this(new TypeDescription((Class)checkRoot(theRoot)));
    }
    ...
    public Constructor(final TypeDescription theRoot, final Collection<TypeDescription> moreTDs) {
        ...
        this.yamlConstructors.put(null, (Construct)new ConstructYamlObject());
        if (!Object.class.equals(theRoot.getType())) {
            this.rootTag = new Tag(theRoot.getType());
        }
        this.yamlClassConstructors.put(NodeId.scalar, (Construct)new ConstructScalar());
        this.yamlClassConstructors.put(NodeId.mapping, (Construct)new ConstructMapping());
        this.yamlClassConstructors.put(NodeId.sequence, (Construct)new ConstructSequence());
    ...
    protected class ConstructMapping implements Construct
    {
        ...
        protected Object constructJavaBean2ndStep(final MappingNode node, final Object object) {
            ...
            final Class<?> beanType = (Class<?>)node.getType();
            final List<NodeTuple> nodeValue = (List<NodeTuple>)node.getValue();
            for (final NodeTuple tuple : nodeValue) {
                ...
                try {
                    ...
                    if (!typeDetected && valueNode.getNodeId() != NodeId.scalar) {
                        ...
                        if (arguments != null && arguments.length > 0) {
                            if (valueNode.getNodeId() == NodeId.sequence) {
                                ...
                                mnode2.setTypes((Class)ketType, (Class)valueType);
                    ...
                    if (property.getType() == String.class && Tag.BINARY.equals((Object)valueNode.getTag()) && value instanceof byte[]) {
        ...
        protected Property getProperty(final Class<?> type, final String name) {
            return Constructor.this.getPropertyUtils().getProperty((Class)type, name);
        }
    }
    ...
    protected class ConstructScalar extends AbstractConstruct
    {
        public Object construct(final Node nnode) {
            ...
            final Class<?> type = (Class<?>)node.getType();
            try {
                return Constructor.this.newInstance(type, (Node)node, false);
            }
            catch (InstantiationException ex) {
                Object result;
                if (type.isPrimitive() || type == String.class || Number.class.isAssignableFrom(type) || type == Boolean.class || Date.class.isAssignableFrom(type) || type == Character.class || type == BigInteger.class || type == BigDecimal.class || Enum.class.isAssignableFrom(type) || Tag.BINARY.equals((Object)node.getTag()) || Calendar.class.isAssignableFrom(type) || type == UUID.class) {
                    ...
                    if (oneArgCount == 1) {
                        ...
                        try {
                            ...
                            throw new YAMLException("Can't construct a java object for scalar " + node.getTag() + "; No String constructor found. Exception=" + e.getMessage(), (Throwable)e);
                        }
                    }
                    try {
        ...
        private Object constructStandardJavaInstance(final Class type, final ScalarNode node) {
            ...
            if (type == String.class) {
                ...
                result = contr2.construct((Node)node);
            }
                    ...
                    if (!possibleConstructors.isEmpty()) {
                        if (possibleConstructors.size() == 1) {
                            ...
                            for (final Node argumentNode : snode.getValue()) {
                                ...
                                argumentNode.setType((Class)type);
