...
public class JavaCodeSerializer implements Serializer
{
    ...
    public String toString(final Reflections reflections) {
        if (reflections.getStore().keys(Utils.index((Class)TypeElementsScanner.class)).isEmpty() && Reflections.log != null) {
            Reflections.log.warn("JavaCodeSerializer needs TypeElementsScanner configured");
        }
        final StringBuilder sb = new StringBuilder();
        List<String> prevPaths = new ArrayList<String>();
        int indent = 1;
        final List<String> keys = new ArrayList<String>(reflections.getStore().keys(Utils.index((Class)TypeElementsScanner.class)));
        ...
        for (final String fqn : keys) {
            ...
            final Iterable<String> members = reflections.getStore().get(Utils.index((Class)TypeElementsScanner.class), fqn);
    ...
    public static Class<?> resolveClassOf(final Class element) throws ClassNotFoundException {
        ...
        final String classOgnl = Utils.join((Collection)ognl.subList(1, ognl.size()), ".").replace(".$", "$");
